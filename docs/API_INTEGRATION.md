...
免费版只自动使用五类经过筛选的 Neural 角色声线，并由应用内部
`voice_id` 确定性映射。中文及中英混合文本使用以下中文声线：

- `Xiaoxiao`：女旁白；
- `Xiaoyi`：成年女性使用原生参数；小女孩复用其更活泼的普通话底声并采用独立童声参数；
- `Yunxi`：更年轻、清亮、适合对白的成年男性；
- `Yunxia`：小男孩，配合轻微童声参数。

更多年龄、方言和定制音色保留在高级 provider 中；未连接付费服务时
在普通用户界面标灰，不会自动分配。

纯英文文本会自动改用与这五类角色 ID 对应的英文 Neural 声线；只含
短英文标题、API 名称或同时含有中文的文本不会切换。语言也进入音频
缓存键，避免同一句文字在中英文路由之间错误复用缓存。

Neural provider 的旁白和成年角色使用原生音高；成年女性和成年男性
分别使用 `-2%` 与 `-1%` 的轻微慢速，不做音高变调；小男孩保持
