...
当前版本是可实际试用的 `0.4 Book Project Alpha`。它不训练 TTS 模型，而是把大模型、角色一致性、多家语音服务、音频拼接和播放器整合成普通人会用的产品：
...
- 无 API Key 时，使用浏览器 Web Speech API 做多人试听；支持 0.8–1.5 倍速、当前说话人提示和从任意一句开始。
...
8. 高品质语音服务已连接时，点击“开始生成”。

离线链路检查、WAV / MP3 格式和供应商选择均放在“导出与开发检查”中，不干扰普通用户主流程。MP3 需要系统安装 `ffmpeg`；仓库里的 Dockerfile 已包含它。
...
- 当前已有 EPUB、多章项目、跨章角色记忆、全书发音记忆、单句重做、WAV / 可选 MP3、片段缓存和失败重试；后台队列和边生成边播放仍在 0.5 阶段。
- 跨章角色合并目前以明确姓名和别名为主；“他 / 父亲 / 李总”等隐含指代仍需要千问上下文与人工确认。
