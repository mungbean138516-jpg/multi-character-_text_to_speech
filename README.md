...
当前版本是可实际试用的 `0.3 Consumer Alpha`。它不训练 TTS 模型，而是把大模型、角色一致性、多家语音服务、音频拼接和播放器整合成普通人会用的产品：
...
4. 试听角色声音；有需要时直接更换。
5. 点击“从头试听”，或从任意一句开始听。
6. 发现串台或错音时，用句子下方的纠错按钮修正。
7. 高品质语音服务已连接时，点击“开始生成”。
...
- 当前已有全书发音记忆、单句重做、WAV / 可选 MP3、片段缓存和失败重试；EPUB、后台队列、边生成边播放和多章项目仍在后续阶段。
