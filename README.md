...
当前版本是可实际试用的 `0.5 Progressive Playback Alpha`。它不训练 TTS 模型，而是把大模型、角色一致性、多家语音服务、音频拼接和播放器整合成普通人会用的产品：
...
8. 高品质语音服务已连接时，点击“开始生成”；第一批句子完成后即可播放。
