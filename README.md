...
- 安装可选 Neural 声线包后，免费版使用 5 种精选普通话 / 国语声线生成、缓存、渐进播放和导出真实朗读；取消 Neural 音高变调，老年角色自然回退到对应性别的成年声线。
...
重启后页面会出现“免费高质量生成”。免费版只保留五种常用角色声线：

- 旁白：`Xiaoxiao`；
- 成年女性：`Xiaoyi`；
- 成年男性：`Yunyang`，采用专业叙事取向；
- 小女孩：`HsiaoYu`，保留当前盲听表现最好的配置；
- 小男孩：`Yunxi`，使用其原生年轻男声，不再人工抬高音调；
- 老人、方言和更多定制角色在界面中标为高级声线；免费生成遇到老年角色时使用自然成年声，不再制造“鬼声”。

Neural 模式不再修改音高，只保留 `-3%` 到 `+2%` 的轻微语速差异。
...
- 设备声音效果音规避、Neural 原生音高与角色试听、Mac 本地中文语音转换；
- 调用前的字符限额。
