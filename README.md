...
- 安装可选 Neural 声线包后，免费版使用 5 种精选普通话 / 国语声线生成、缓存、渐进播放和导出真实朗读；成年人保持原生音高，儿童仅做轻微童声调节，老年角色自然回退到对应性别的成年声线。
...
- 小女孩：`HsiaoYu`，恢复上一版盲听表现更好的轻快童声配置；
- 小男孩：`Yunxia`，使用更稚嫩的年轻男声；
- 老人、方言和更多定制角色在界面中标为高级声线；免费生成遇到老年角色时使用自然成年声，不再制造“鬼声”。

旁白与成年角色使用 Neural 原生音高；小女孩和小男孩只使用 `+2Hz` 的轻微童声调节与 `+4%` 语速，避免把成年声硬拉成夸张假童声。
