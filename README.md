...
- 导入 DOCX 或可提取文字的 PDF，并按“第一章 / Chapter 1”等标题自动拆章；扫描版 PDF 需先 OCR。
...
- 免费版只自动分配旁白、小女孩、小男孩、成年女性和成年男性 5 类精选声线；其余同模型 CosyVoice 音色在未接入付费服务时标灰，接入后才可选择和试听。
...
- 无 API Key 时，整段“快速试听”使用设备声音；角色卡的“Neural 试听”会只生成该角色的一句话，并与完整生成复用同一声音和缓存。
- 安装可选 Neural 声线包后，免费版使用 5 类精选普通话 / 国语角色声线生成、缓存、渐进播放和导出真实朗读；成年人保持原生音高，儿童使用独立童声调节，老年角色自然回退到对应性别的成年声线。
...
- 运行环境只需 Python 3.10+；PDF 文字提取使用自动安装的轻量依赖 `pypdf`，DOCX 仍由标准库解析，高质量免费声线作为可选安装。
...
4. 第一次运行会在项目内创建独立环境并安装免费 Neural 中英文声线，完成后自动打开浏览器；
5. 以后只需再次双击同一个文件。
...
1. 点击“载入原创演示文本”。
2. 或把自己的合法 TXT / EPUB / PDF / DOCX 拖入输入框。
3. EPUB 导入后选择一章，点击“自动识别角色”。
4. 点击角色卡上的“Neural 试听”，听到完整生成时会使用的同一声音；有需要时直接更换。
5. 切到下一章继续识别，已经确认的角色和声音会自动沿用。
6. 点击“Neural 连续试听”，或点击任意一句的“从这里听 Neural”；这些句子会与最终生成共用缓存。
7. 发现串台或错音时，用句子下方的纠错按钮修正。
8. 点击“用这 5 类声音生成”；断网时可在 Mac 上改用“备用：Mac 本地声音”。第一批句子完成后即可播放。

WAV / MP3 格式等选项放在“导出与开发检查”中，不干扰普通用户主流程。MP3 需要系统安装 `ffmpeg`；仓库里的 Dockerfile 已包含它。

如果 Mac 没有显示“备用：Mac 本地声音”，请前往“系统设置 → 辅助功能 → 朗读内容”，下载至少一个中文系统声音后重启本项目。Apple 的官方步骤见[更改 Mac 用于朗读文本的声音](https://support.apple.com/guide/mac-help/mchlp2290/mac)。
...
重启后页面会出现“Neural 连续试听”和“用这 5 类声音生成”。免费版只保留五类常用角色声线；纯英文文本自动切换到对应的英文 Neural 声线，中文及中英混合内容继续使用中文声线：

- 旁白：`Xiaoxiao`；
- 成年女性：`Xiaoyi`；
- 成年男性：`Yunxi`，采用更年轻、清亮、适合对白的普通话男声；
- 小女孩：`Xiaoyi`，使用更活泼的普通话底声，并加强为清亮但不过度卡通的童声配置；
- 小男孩：`Yunxia`，使用更稚嫩的年轻男声；
- 老人、方言和更多定制角色在界面中标为高级声线；免费生成遇到老年角色时使用自然成年声，不再制造“鬼声”。

旁白与成年角色使用 Neural 原生音高；林夏类成年女声使用 `-2%`，陈默类成年男声使用 `-1%` 的舒展语速。小男孩保持 `+2Hz / +4%`，小女孩使用 `+8Hz / +6%`，让性别和年龄更容易听出来，同时避免极端变调。

英文声线沿用相同的五类角色 ID，因此角色锁定、别名与跨章节声音一致性无需另建一套逻辑。纯英文检测要求至少 20 个英文字母且不能出现中日韩统一表意文字，避免把短标题、API 名称或中英混合小说误切到英文声线。
...
audiobook_app/
  ...
  providers/
    ...
    neural.py       免 Key 在线 Neural 中英文声线与自然角色映射
...
| `POST` | `/api/import/document` | PDF / DOCX 二进制 → 自动拆章的声场书籍项目 |
...
- DOCX 自动拆章、全书角色一致性检查、规则导演与相似声线替换入口；
